...
class TestWorkDataRouter:
  ...
  async def test_update_account_email(
          ...
          user_test,
          clean_data, add_users,
          add_accounts, add_secrets
  ) -> None:
    with expectation:
      await clean_data()
      await add_users()
  ...
  @staticmethod
  @pytest.mark.parametrize(
  ('url', 'first_name', 'last_name', 'json',  'expected_status_code', 'expected_payload', 'expectation'),
  ...
  async def test_update_user_first_and_last_name(
          ...
          async_client: AsyncClient,
          user_test,
          clean_data, add_users, add_companies,
          add_accounts, add_secrets, add_members
  ) -> None:
    with expectation:
      await clean_data()
      await add_users()
      ...
      response = await async_client.put(url, json=json, headers=headers)
      assert response.status_code == expected_status_code
      assert prepare_payload(response, ['id','registered_at', 'updated_at']) == expected_payload
