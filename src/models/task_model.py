...
from sqlalchemy import String, ForeignKey, UniqueConstraint, Table, Column, Enum
...
class TaskModel(BaseModel):
        ...
        observers: Mapped[list["User"]] = relationship(secondary=observers_table, lazy="selectin")
        performers: Mapped[list["User"]] = relationship(secondary=performers_table, lazy="selectin")
        created_at: Mapped[created_at_ct]
        updated_at: Mapped[updated_at_ct]
        deadline: Mapped[str]
        status: Mapped[Enum] = mapped_column(Enum(TaskStatus), default=TaskStatus.IN_PROCESS)
