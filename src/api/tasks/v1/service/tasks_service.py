...
class TasksService(BaseService):
  ...
  async def get_task_by_id(self, task_id: UUID4) -> TaskDB:
    ...
    Get task by id
    """
    task: TaskModel | None = await self.uow.task.get_by_query_one_or_none(id=task_id)
  ...
  async def create_task(self, task_data: dict) -> TaskDB:
    ...
    task_with_users: TaskModel = await self.uow.task.add_users_to_task_and_get_obj(task=task, observers=observers, performers=performers)
  ...
  async def update_task(self, task_id: UUID4, task_data: dict)  -> TaskDB:
    ...
    """
    observers, performers = await self.add_observers_and_performers(task_data)
    task: TaskModel | None = await self.uow.task.update_one_by_id(_id=task_id, **task_data)
    updated_task: TaskModel = await self.uow.task.update_users_to_task_and_get_obj(task=task, observers=observers, performers=performers)
  ...
  async def delete_all_tasks(self) -> None:
    ...
    Delete all tasks
    """
    await self.uow.task.delete_all()

  async def add_observers_and_performers(self, task_data: dict) -> tuple[Sequence, Sequence]:
    """
    Add observers and performers to a task
    """
    observers = await self.uow.user.get_by_query_all_in_users_list(task_data.get("observers"))
    self._check_users_exists(observers)
    performers = await self.uow.user.get_by_query_all_in_users_list(task_data.get("performers"))
  ...
  def _check_task_exists(task: TaskModel | None) -> None:
    """
    Сhecking the availability of a task in the database
    """
    if not task:
  ...
  def _check_users_exists(users: Sequence[User]) -> None:
    """
    Сhecking the availability of users in the database
    """
    if not users:
