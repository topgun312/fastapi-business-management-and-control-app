...
from src.schemas.user_schema import CreateUserRequest, UserAuthSchema, UserDB, UserResponse, UserCreateResponse
...
@cache(expire=3600)
async def get_member_info_by_user_id(user_id: UUID4,
                                    admin: UserAuthSchema = Depends(get_current_admin_auth_user),
                                    service: MemberService = Depends(MemberService)) -> UserResponse:
