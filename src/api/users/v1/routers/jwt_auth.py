...
from api.users.v1.auth_utils.validate import (
    ...
    validate_auth_user, get_current_auth_user_for_refresh,
)

from src.schemas.user_schema import UserAuthSchema, Token
...
async def auth_user_issue_jwt(user: UserAuthSchema = Depends(validate_auth_user)) -> Token:
    """
    Auth user issue JWT
    """
...
async def auth_refresh_jwt(user: UserAuthSchema = Depends(get_current_auth_user_for_refresh)) -> Token:
    """
    Auth refresh JWT
    """
    access_token = create_access_token(user)
