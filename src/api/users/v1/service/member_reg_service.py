...
class MemberService(BaseService):
    ...
    async def add_member(
      ...
      """
      admin_account: AccountModel = await self.uow.account.get_by_query_one_or_none(user_id=admin.id)
      check_account: AccountModel = await self.uow.account.get_by_query_one_or_none(email=email.get('email'))
      self._check_account_already_exists(check_account)
      user: User = await self.uow.user.add_one_and_get_obj(
                    **user_data,
                )
      account: AccountModel = await self.uow.account.add_one_and_get_obj(user_id=user.id, email=email.get('email'))
      company: CompanyModel = await self.uow.company.get_by_query_one_or_none(account_id=admin_account.id)
    ...
    def _check_account_already_exists(check_account: AccountModel | None) -> None:
        if check_account is not None:
            raise HTTPException(
                status_code=status.HTTP_400_BAD_REQUEST, detail='Such an email already exists',
            )
