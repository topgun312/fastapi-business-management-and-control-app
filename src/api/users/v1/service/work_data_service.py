...
from schemas.account_schema import AccountDB
...
class WorkDataService(BaseService):
    ...
    async def update_email(self, _email: EmailStr, email: dict) -> AccountDB:
      ...
      """
      account = await self.uow.account.get_by_query_one_or_none(email=_email)
      self._check_account_exists(account)
      result = await self.uow.account.update_one_by_email(
    ...
    async def update_user_data(
      ...
      new_user = await self.uow.user.update_one_by_id(
