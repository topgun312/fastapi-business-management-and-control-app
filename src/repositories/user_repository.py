...
class UserRepository(SQLAlchemyRepository):
    ...
    async def get_user_by_email(self, username: EmailStr) -> type(model) | None:
        query = select(self.model).join(AccountModel.user).where(AccountModel.email == username)
        res: Result = await self.session.execute(query)
        return res.unique().scalar_one_or_none()

    async def get_by_query_all_in_users_list(self, task_data: dict) -> Sequence[type(model)]:
